...
namespace Persistencia
{
    internal class Per_Alumnos : Inter_Alumnos
    {
        ...
        public List<Inscripcion> Listar()
        {
            ...
            List<Inscripcion> lista = new List<EntidadesCompartidas.Inscripcion>();

            SqlDataReader oReader;
            SqlConnection oConexion = new SqlConnection(Conexion.Cnn);

            SqlCommand oComando = new SqlCommand("ListarInscripcion ", oConexion);
            oComando.CommandType = CommandType.StoredProcedure;
            try
            {
                oConexion.Open();
                ...
                while (oReader.Read())
                {
                    fechainscr = (DateTime)oReader["FechaInscripcion"];
                    cedAlu = (int)oReader["Alumno"];
                    cod = (string)oReader["Codigo"];

                    alu = FabricaPersistencia.getPersistenciaAlumnos().Buscar(cedAlu);
                    curso = FabricaPersistencia.getPersistenciaCursoEspecializado().Buscar(cod);
                    if(curso == null)
                    curso = FabricaPersistencia.getPersistenciaCursosCortos().Buscar(cod);

                    Inscripcion inns = new Inscripcion(alu, fechainscr, curso);
